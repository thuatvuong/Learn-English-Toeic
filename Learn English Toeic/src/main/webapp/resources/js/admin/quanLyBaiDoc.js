$(document).ready(function(){
	...
	function ajaxGet(page){
		...
		$.ajax({
			type: "GET",		
			url: "/api/admin/bai-doc/all" + "?page=" + page + "&doKho=" + doKho + "&part=" + part,
...
//					console.log(baiDoc);
	...
	$(document).on('click', '.btnCapNhatBaiDoc', function(event) {
		...
		var href = "/api/admin/bai-doc/"+baiDocId;
		$.get(href, function(baiDoc) {
            ...
            $("img").attr("src", "/file/images/baiDocId="+ baiDoc.id+".png");
            $("#previewImage").removeClass("hidden");
            $("#linkExcel").attr("href", "/file/excel/baiDocId="+ baiDoc.id+".xlsx");
    ...
    $(document).on("click",".btnXoaBaiDoc", function() {		
		...
		if(confirmation){
		  $.ajax({
			  type : "DELETE",
			  url : "/api/admin/bai-doc/delete/" + baiDocId,
    ...
    function resetForm(){
 	...
 	function updateFunction(formData, id) {
		...
		$.ajax({
			...
			type : "POST",
			contentType : "application/json",
			url : "/api/admin/bai-doc/update/" + id,
			enctype : 'multipart/form-data',
			data : formData,
			// prevent jQuery from automatically transforming the data into a
			// query string
